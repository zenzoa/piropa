...
#include "sprites/bugs_small/cicada.h"
...
static uint8_t cicada_scene[CICADA_COUNT];
static uint8_t cicada_x[CICADA_COUNT];
static uint8_t cicada_y[CICADA_COUNT];
static uint8_t cicada_goal_x[BUTTERFLY_COUNT];
static uint8_t cicada_goal_y[BUTTERFLY_COUNT];
static uint8_t cicada_anim_counter[CICADA_COUNT];
static uint8_t cicada_anim_frame[CICADA_COUNT];
static uint8_t cicada_anim_speed[CICADA_COUNT];
static uint8_t cicada_run_away[CICADA_COUNT];
...
static uint8_t fly_respawn_time(void) {
	return 2 * (game_speed + 1);
}

static uint8_t dragonfly_respawn_time(void) {
	return 4 * (game_speed + 1);
}

static uint8_t firefly_respawn_time(void) {
	return 4 * (game_speed + 1);
}

static uint8_t butterfly_respawn_time(void) {
	return 4 * (game_speed + 1);
}
...
static void spawn_cicada(uint8_t i, uint8_t is_setup) {
	if (life_stage == DEAD) { return; }
	cicada_alive[i] = TRUE;
	cicada_respawn[i] = 0;
	...
	cicada_anim_speed[i] = 64 + rand() % 128;
	cicada_run_away[i] = FALSE;
}
...
static void draw_cicada(uint8_t x, uint8_t y, uint8_t frame, uint8_t flip) {
	if (flip) {
		draw_banked_sprite_flip(BANK(cicada), (uint8_t *) cicada_metasprites, frame, CICADA_VRAM, BROWN_PALETTE, x + 8, y);
	} else {
		draw_banked_sprite(BANK(cicada), (uint8_t *) cicada_metasprites, frame, CICADA_VRAM, BROWN_PALETTE, x, y);
	}
}
...
static void update_fly(uint8_t i) {
	...
	if (fly_anim_counter[i] >= 12) {
		...
		if (fly_x[i] == 0 || fly_x[i] > 168 || fly_y[i] < MIN_Y || fly_y[i] > 144) {
			fly_alive[i] = FALSE;
			fly_respawn[i] = fly_respawn_time();
		}
...
static void update_dragonfly(uint8_t i) {
	...
	if (dragonfly_anim_counter[i] % 2) {
		...
		if (dragonfly_x[i] == 0 || dragonfly_x[i] > 168 || dragonfly_y[i] < MIN_Y || dragonfly_y[i] > 144) {
			...
			dragonfly_respawn[i] = dragonfly_respawn_time();
		}
	}
}
...
static void update_firefly(uint8_t i) {
	...
	if (firefly_anim_counter[i] >= 8) {
		...
		if (firefly_x[i] == 0 || firefly_x[i] > 168 || firefly_y[i] < MIN_Y || firefly_y[i] > 144) {
			...
			firefly_respawn[i] = firefly_respawn_time();
		}
...
static void update_butterfly(uint8_t i) {
	...
	if (butterfly_anim_counter[i] >= frame_rate) {
		...
		if (butterfly_x[i] == 0 || butterfly_x[i] > 168 || butterfly_y[i] < MIN_Y || butterfly_y[i] > 144) {
			...
			butterfly_respawn[i] = butterfly_respawn_time();
		}

	}
}

void update_cicada(uint8_t i) {
	cicada_anim_counter[i] += 1;
	if (cicada_run_away[i]) {
		if (cicada_anim_counter[i] >= 6) {
			cicada_anim_counter[i] = 0;
			cicada_anim_frame[i] = !cicada_anim_frame[i];
			cicada_x[i] -= 1;
			cicada_y[i] -= 1;
		}
	} else if (cicada_x[i] != cicada_goal_x[i] || cicada_y[i] != cicada_goal_y[i]) {
		if (cicada_anim_counter[i] >= 6) {
			cicada_anim_counter[i] = 0;
			cicada_anim_frame[i] = !cicada_anim_frame[i];
			if (cicada_x[i] < cicada_goal_x[i]) {
				cicada_x[i] += 1;
			}
			if (cicada_y[i] < cicada_goal_y[i]) {
				cicada_y[i] += 1;
			}
		}
	} else if (cicada_anim_counter[i] >= cicada_anim_speed[i]) {
		cicada_anim_counter[i] = 0;
		cicada_anim_frame[i] = !cicada_anim_frame[i];
		cicada_anim_speed[i] = 64 + rand() % 128;
	}
	if (cicada_x[i] == 0 || cicada_x[i] > 168 || cicada_y[i] < MIN_Y || cicada_y[i] > 144) {
		cicada_alive[i] = FALSE;
		cicada_respawn[i] = cicada_respawn_time();
	}
}
...
void draw_bugs(void) BANKED {
	...
	for (uint8_t i = 0; i < CICADA_COUNT; i++) {
		if (cicada_alive[i] &&
			cicada_scene[i] == current_scene) {
				update_cicada(i);
				draw_cicada(cicada_x[i], cicada_y[i], cicada_anim_frame[i], FALSE);
		} else if (i < CICADA_COUNT - 1 &&
			!cicada_alive[i] && !cicada_respawn[i] &&
			current_scene == FIELD && !is_night) {
				spawn_cicada(i, FALSE);
		}
	}
}
...
uint8_t grab_bug(void) BANKED {
	...
	for (uint8_t i = 0; i < CICADA_COUNT; i++) {
		if (cicada_alive[i] && cicada_scene[i] == current_scene &&
			hand_x + 8 >= cicada_x[i] && hand_x < cicada_x[i] + 8 &&
			hand_y + 8 >= cicada_y[i] && hand_y < cicada_y[i] + 8) {
				cicada_alive[i] = FALSE;
				cicada_respawn[i] = cicada_respawn_time();
				cicada_run_away[i] = TRUE;
				return BUG_CICADA;
		}
	}
...
void setup_bugs_data(void) BANKED {
	...
	set_banked_sprite_data(BANK(cicada), CICADA_VRAM, cicada_TILE_COUNT, cicada_tiles);
}
